-- Fix: column reference "id" is ambiguous in cancel_invoice and replace_invoice functions.
-- The function parameter is named "id" but several queries used bare "id" which
-- PostgreSQL couldn't resolve between the parameter and table columns.
-- All references now use fully-qualified names (e.g., invoices.id or cancel_invoice.id).
...
begin
  ...
  -- Use alias for table to avoid ambiguity with function parameter 'id'
