...
import { parseScannedBarcode } from '@/lib/scanCodes'
import {
  ...
  useBarcodeScanner({
    onScan: (barcode) => {
      void (async () => {
        ...
        // Use parseScannedBarcode to properly detect batch/bag labels.
        // This handles both prefixed (B1073, G0015) AND pure numeric (000001073) barcodes
        // that are generated by getBatchBarcodeValue / getBagBarcodeValue.
        const parsed = parseScannedBarcode(logical)
        
        if (parsed.kind === 'batch') {
          const token = `B${parsed.batchId}`
          showToast('تم اكتشاف ملصق الدفعة — فتح شاشة الفاتورة', 'info', 3200)
          setInvoiceNewPendingBarcode(token)
          navigate(`/invoices/new?barcode=${encodeURIComponent(token)}`)
          return
        }
        if (parsed.kind === 'bag') {
          const token = `G${parsed.bagInstanceId}`
          showToast('تم اكتشاف ملصق الكيس — فتح شاشة الفاتورة', 'info', 3200)
          setInvoiceNewPendingBarcode(token)
          navigate(`/invoices/new?barcode=${encodeURIComponent(token)}`)
          return
        }
        ...
        // For all other pages, navigate to the new invoice screen
        setInvoiceNewPendingBarcode(logical)
